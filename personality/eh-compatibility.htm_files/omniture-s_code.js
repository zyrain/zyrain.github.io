...
s.linkInternalFilters="javascript:,eharmony.com"
...
s.useCommerce=true
s.varUsed="prop25"
s.eventList="event6" //Abandon,Success,Error
...
/* AB Testing */
if((!s.prop31) && (typeof eh_contentstring != "undefined"))
	s.prop31=eh_contentstring
	
/* External Campaign Tracking */
if(!s.campaign)
	s.campaign=s.getQueryParam('cid,aid',':')
	s.campaign=s.getValOnce(s.campaign,'s_campaign',0)

/* Content Variables */
if(!s.prop1)
	s.prop1=s.getQueryParam('keyword')
if(!s.prop5)
	s.prop5=s.getQueryParam('page')
if(!s.prop7)
	s.prop7=s.getQueryParam('AID,aid',':')
if(!s.prop8)
	s.prop8=s.getQueryParam('CID',':')
	s.prop8=s.getQueryParam('CID');

if(!s.prop9)
	s.prop9=s.getQueryParam('FID,fid',':')
if(!s.prop10)
	s.prop10=s.getQueryParam('TID,tid',':')
if(!s.prop33)
	s.prop33=s.getQueryParam('PID,pid',':')	
if(!s.eVar22)
	s.eVar22=s.getQueryParam('username')
...
if (!s.eVar27) {s.eVar27=s.getQueryParam('PURCHASE_REASON');}
if (s.eVar27) {s.events=s.apl(s.events,"event20",",",2);}

s.prop22=document.location;
...
s.prop11=s.getTimeParting('h','-8','2008'); // Set hour 
s.prop12=s.getTimeParting('d','-8','2008'); // Set day
s.prop13=s.getTimeParting('w','-8','2008'); // Set Weekend / Weekday

var rURL = document.referrer;
if (rURL.match('advice.eharmony.com')) { s.prop34="advice.eharmony.com";}
